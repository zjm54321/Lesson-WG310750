...
static volatile float adc_value = 0;
static uint16_t adc_fpu8 = 0;

int32_t main(void) {
    float adc_value_pre = 0;
    ...
    for (;;) {
        if (adc_value != adc_value_pre) {
            ...
            adc_fpu8 = (uint16_t)(adc_value * 100) % 1000;
            uint8_t buffer[16];
            format_adc(adc_value, buffer);
            send_serial(buffer);
        }
    }
}
...
void timer1_isr(void) interrupt(3) {
    ...
    if (++count >= 50) {
        count = 0;
        adc_value = rd_temperature_f(); // 读取ADC值（Rb2）
    }
}
